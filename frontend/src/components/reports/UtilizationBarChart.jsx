...
export default function UtilizationBarChart({ data, formatValue = (v) => `${v}%`, tooltipSuffix = 'utilized' }) {
  const [hovered, setHovered] = useState(null)
  const step = CHART_WIDTH / data.length
  const max = Math.max(...data.map((d) => d.value), 1)
  const scale = (value) => BASELINE_Y - ((BASELINE_Y - TOP_Y) * value) / max
    ...
    <div className="relative">
      <svg viewBox={`0 0 ${CHART_WIDTH} 210`} className="w-full" role="img" aria-label="Utilization by department bar chart">
        ...
        {data.map((d, i) => {
          ...
          const isHigh = d.value >= max * HIGH_THRESHOLD_RATIO
      ...
      <div className="mt-1 flex" style={{ paddingLeft: 0 }}>
        {data.map((d) => (
          <div key={d.label} className="truncate px-0.5 text-center text-xs font-medium text-slate-500 dark:text-slate-400" style={{ width: `${100 / data.length}%` }}>
            {d.label}
          </div>
        ))}
      </div>
      ...
      {hovered !== null && (
        ...
        >
          ...
          <span className="text-slate-500 dark:text-slate-400">{formatValue(data[hovered].value)} {tooltipSuffix}</span>
        </div>
      )}
    </div>
  )
